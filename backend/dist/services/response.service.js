...
exports.responseService = exports.ResponseService = void 0;
...
class ResponseService {
    ...
    async saveResponse(data) {
        ...
        try {
            ...
            if (data.responseOptionIndex !== undefined) {
                const optionResult = await client.query(`SELECT is_correct FROM question_options
           WHERE question_id = $1 AND option_index = $2`, [data.questionId, data.responseOptionIndex]);
                if (optionResult.rows.length > 0) {
                    isCorrect = optionResult.rows[0].is_correct;
                }
            }
            // Upsert response
            const result = await client.query(`INSERT INTO responses (
        ...
        RETURNING *`, [data.sessionId, data.questionId, data.responseText, data.responseOptionIndex, isCorrect]);
    ...
    async getSessionResponses(sessionId) {
       ...
       ORDER BY q.question_number`, [sessionId]);
        return result.rows;
    }
    /**
     * Get detailed responses with question and option data
     */
    async getDetailedResponses(sessionId) {
        const responses = await database_1.pool.query(`SELECT
        r.*,
        q.question_number,
        q.question_text,
        q.question_type,
       ...
       ORDER BY q.question_number`, [sessionId]);
        ...
        const detailedResponses = await Promise.all(responses.rows.map(async (response) => {
...
exports.responseService = new ResponseService();
