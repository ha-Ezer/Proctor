...
export class SessionService {
  ...
  async createSession(data: CreateSessionData) {
    ...
    try {
      ...
      const examResult = await client.query(
        'SELECT duration_minutes FROM exams WHERE id = $1 AND is_active = true',
        [data.examId]
      );
      ...
      const result = await client.query(
        ...
        RETURNING *`,
        [sessionId, data.studentId, data.examId, scheduledEndTime, data.browserInfo, data.ipAddress]
      );
      ...
      await client.query(
        ...
        ) VALUES ($1, 'Exam started', 'low', 'Student initiated exam session', $2)`,
        [result.rows[0].id, data.browserInfo]
      );
   ...
   */
  async saveSnapshot(sessionId: string, snapshotData: SessionSnapshot) {
    const result = await pool.query(
   ...
   * Generate proctoring report
   */
  private async generateProctoringReport(client: any, sessionId: string, session: any, score: number) {
    // Get violations
    ...
    const violationTypes = [...new Set(violations.map((v: any) => v.violation_type))];

    // Determine status
    let status = 'clean';
    const seriousViolations = violations.filter(
      (v: any) =>
        v.severity === 'critical' || v.severity === 'high' || v.violation_type.toLowerCase().includes('developer tools')
    );
   ...
   */
  async checkViolationLimit(sessionId: string): Promise<boolean> {
    const result = await pool.query(
      `SELECT
        es.total_violations,
        e.max_violations
   ...
   */
  async markSessionResumed(sessionId: string) {
    await pool.query(
      `UPDATE exam_sessions
    ...
    await pool.query(
      ...
      ) VALUES ($1, 'Exam resumed from URL progress', 'low', 'Student recovered and resumed exam')`,
      [sessionId]
    );
  }
   ...
   */
  async checkExistingSession(studentId: string, examId: string) {
    const result = await pool.query(
      `SELECT
        es.*,
        e.title as exam_title,
   ...
   */
  async getRecoveryData(sessionId: string) {
